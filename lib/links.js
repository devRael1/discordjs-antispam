...
const globalLinkRegex = /https?:\/\/(www\.)?[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_\+.~#?&//=]*)/gm;
...
class LinksFilterSystem {
    ...
    async hasDiscordInviteLink (message) {
        ...
        return !!str.match(discordInviteRegex);
    }
    ...
    async hasGlobalLink (message) {
        const str = await this.sanitize(message.cleanContent);
        return !!str.match(globalLinkRegex);
    }
    ...
    async addLinks (links, guild_id) {
        ...
        const add = (word) => {
            if (guild_links.indexOf(word) === -1) {
                guild_links.push(word);
                return true;
            } else return false;
        }
