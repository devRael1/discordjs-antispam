...
class SanctionsManager  {
    ...
    async appliedSanction (sanction, message, spamMessages, options) {
        ...
        switch (sanction) {
            case this.antispam_client.type_sanctions.ban: {
                if (!message.member.bannable) {
                    ...
                    if (options.message.ban) {
                        await message.channel.send(this.antispam_client.logs.format(options.message.ban, message, 'Spamming')).catch(async (e) => {
                            if (options.verbose) {
                                await this.antispam_client.logs.logsError(`DAntiSpam (banUser#sendMissingPermMessage): ${e.message}`, options);
                            }
                        });
                    ...
                    if (options.modLogsEnabled) {
                        await this.logs.logs(message, `banned`, options);
                    }
                    return true;
                }
            }
            case this.antispam_client.type_sanctions.kick: {
                if (!message.member.kickable) {
                    ...
                    if (options.message.kick) {
                        await message.channel.send(this.antispam_client.logs.format(options.message.kick, message, 'Spamming')).catch(async (e) => {
                            if (options.verbose) {
                    ...
                    if (options.modLogsEnabled) {
                        await this.logs.logs(message, `kicked`, options);
                    }
                    return true;
                }
            }
            case this.antispam_client.type_sanctions.mute: {
                ...
                if (options.message.mute) {
                    await message.channel.send(this.antispam_client.logs.format(options.message.mute, message, 'Spamming')).catch(async (e) => {
                        if (options.verbose) {
                ...
                if (options.modLogsEnabled) {
                    await this.logs.logs(message, `muted`, options);
                }
                return true;
            }
            case this.antispam_client.type_sanctions.warn: {
                if (options.message.warn) {
                    await message.channel.send(this.antispam_client.logs.format(options.message.warn, message, 'Spamming')).catch(async (e) => {
                        if (options.verbose) {
                ...
                if (options.modLogsEnabled) {
                    await this.logs.logs(message, `warned`, options);
                }
                return true;
            }
        }
