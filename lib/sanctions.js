        ...
        switch (sanction) {
            case 'ban': {
                if (!message.member.bannable) {
                    ...
                    if (options.errorMessage.enabled) {
                        await message.channel.send(this.format(options.errorMessage.ban, message)).catch(async (e) => {
                            if (options.verbose) {
            ...
            case 'kick': {
                if (!message.member.kickable) {
                    ...
                    if (options.errorMessage.enabled) {
                        await message.channel.send(this.format(options.errorMessage.kick, message)).catch(async (e) => {
                            if (options.verbose) {
            ...
            case 'mute': {
                ...
                if (!userCanBeMuted) {
                    ...
                    if (options.errorMessage.enabled) {
                        await message.channel.send(this.format(options.errorMessage.mute, message)).catch(async (e) => {
                            if (options.verbose) {
