        ...
        switch (sanction) {
            case 'ban': {
                if (!message.member.bannable) {
                    ...
                    await message.member.ban({
                        ...
                        if (options.errorMessages) {
                            await message.channel.send(this.format(options.banErrorMessage, message)).catch(async () => {
                                if (options.verbose) {
                                    await this.logsError(`DAntiSpam (banUser#sendSuccessMessage): ${e.message}`, options);
                                }
                            });
                        }
                    })
            ...
            case 'kick': {
                if (!message.member.kickable) {
                    ...
                    if (options.kickMessage) {
                        await message.channel.send(this.format(options.kickMessage, message)).catch(async (e) => {
                            if (options.verbose) {
            ...
            case 'mute': {
                ...
                if (options.muteMessage) {
                    await message.channel.send(this.format(options.muteMessage, message)).catch(async (e) => {
                        if (options.verbose) {
            ...
            case 'warn': {
                if (options.warnMessage) {
                    await message.channel.send(this.format(options.warnMessage, message)).catch(async (e) => {
                        if (options.verbose) {
